...
<div class="card text-center">
        <div class="card-header">
            <ul class="nav nav-tabs card-header-tabs">
                <li class="nav-item">
                    <a class="nav-link active "  href="inicioConductor.php" >Reservaciones</a>
                </li>
                <li class="nav-item">
...
$fecha= date('Y-m-d');
 ...
 $resultado=mysqli_query($conexion,"select * from reservacion WHERE reservacion.fecha='$fecha'");
 $todo="";
 while($row=$resultado->fetch_array()){
     $idRe=$row['idVehiculo'];
     $idRese=$row['idReservacion'];
     $nombre=$row['nombre'];
     $idCajon=$row['idCajon'];
     $hora=$row['hora'];
     $fechaR=$row['fecha'];
    ...
    $todo .= "<tr> <td>" .$nombre ."</td> <td>". $idCajon."</td> <td>". $hora." </td> <td>". $fechaR." </td> <td>". $lavado1.
    " </td> <td> <a class='btn btn-primary' href='savaRecepCajera.php?idReser=$idRese & idVe=$idRe&cajon=$idCajon&hora=$hora&fecha=$fechaR&lavado=$lavado'>Resguardar</a>".
    " <a class='btn btn-primary' href='cajeraCancelaraRes.php?idRes=$idRese'>Cancelar</a> </td> </tr>";
